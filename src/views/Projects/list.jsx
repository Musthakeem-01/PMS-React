...
export default function CustomTable(props) {
    ...
    async function fetchData(){
        ...
        const params = { "data": { "p1_int": 4, "p2_int": weekNumber, "p3_int": year, "p4_int": null, "p5_int": null, "p6_int": eid } }

        const response = await apiPostRequest(url, params);

        console.log('res',response)
        const { Output: { status: { code }, data } } = response;


        let hiddenColumns = ["IDPK", "ID", "CreatedDate", "UpdatedDate", "DeleStat", "UpdatedTtm", "CreatedTtm", "Code"];
        let total = {
          "7": 0,
          "14": 0,
          "30": 0,
    ...
    function numberToColorClass(number) {
        const colors = ["50", "50", "100", "100", "200", "200", "300", "300", "400", "400"];
    ...
    <div data-aos="zoom-in" className="flex flex-col w-full p-1 aos-init aos-animate " style={{height:'60vh'}}> 

       <div id="generate_table_html_cwise" className="flex-grow px-0 overflow-auto scroll-smooth custom-scrollbar shadow-md rounded-lg bg-white ">
        <table className="relative tracking-wider w-full border text-left text-xs text-slate-500">
          ...
          <tbody id="DynamicTable_CE_WNA" className="divide-y bg-blue-50 text-slate-800 bg-white">
  {TableBody.filter(row => row[Object.keys(row).find(key => key.endsWith("ID"))])
    .map((x, i) => {
      const rowColumns = header.map(({ key }) => {
        if (key.startsWith("Is")) {
          return (
            <td key={key} className="py-1 px-1">
              <input type="checkbox" value={x[key] ? "checked" : ""} />
            </td>
          );
        } else {
          let color =
          ...
          return (
            <td key={key} onClick={() => onClick_Data(weekNumber,year,eid,x["Descriptions"],key,x["WorkOrderID"])}
            ...
            >
              ...
              key === "90" ||
              key === "120" ) ? (
                <p className={`transform hover:scale-150 hover:bg-blue-700 transition-transform duration-300 hover:text-white 
                bg-${color}-${numberToColorClass(x[key])} hover:tr text-center rounded font-semibold`}>
                  {x[key] === 0 ? "" : x[key]}
                </p>
              ) : x[key] === 0 ? (
                ""
      ...
      return (
        <tr
          ...
          className={`truncate border-b hover:bg-blue-100 hover:text-blue-600 cursor-pointer ${
            i === 0 ? "bg-violet-200" : i % 2 !== 0 ? "bg-slate-50" : ""
          }`}
